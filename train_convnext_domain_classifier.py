...
#     def __init__(self, backbone_name: str = "convnext_base", num_classes: int = 2, pretrained: bool = True):
#         super().__init__()
...
#             self.backbone = tv_models.convnext_large(
...
#             self.backbone = tv_models.convnext_small(
...
#         description="Train ConvNeXt-based domain classifier (IAM vs Emuru) on baseline / A1–A5 CSVs."
#     )
#     parser.add_argument(
#         "--project-root",
#         type=str,
...
#         help="Path to domain CSV (relative to project root). Use A4/A5 CSVs for those variants.",
#     )
#     parser.add_argument(
#         "--tag",
#         type=str,
#         default="convnext_baseline",
#         help="Tag for checkpoint naming, e.g. 'convnext_baseline', 'convnext_A1', etc.",
#     )
#     parser.add_argument(
#         "--backbone",
#         type=str,
#         default="convnext_base",
#         help="ConvNeXt backbone: convnext_tiny, convnext_small, convnext_base, convnext_large",
#     )
...
#         "--freeze-backbone",
#         action="store_true",
#         help="If set, freeze ConvNeXt backbone and train only the final head.",
#     )
#     parser.add_argument(
#         "--augment-train",
#         action="store_true",
#         help="A1: small geometric jitter on train split.",
#     )
#     parser.add_argument(
#         "--photo-augment-train",
#         action="store_true",
#         help="A2: photometric jitter + blur on train split.",
#     )
#     parser.add_argument(
#         "--normalize-all",
#         action="store_true",
#         help="A3: contrast normalization for ALL splits.",
#     )
...
#     model = ConvNeXtDomainClassifier(
...
#             if name.startswith("classifier.2"):
#                 param.requires_grad = True
#             else:
#                 param.requires_grad = False
#         print("Backbone frozen; only final classifier will be trained.")
#         optimizer = optim.Adam(
#             filter(lambda p: p.requires_grad, model.parameters()),
#             lr=args.lr,
#         )
#     else:
#         print("Backbone NOT frozen; full model will be trained (heavier).")
...
#         raise RuntimeError("No 'train' split found in CSV. Cannot train ConvNeXt.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train ConvNeXt-based domain classifier (IAM vs Emuru) on Baseline/A1–A5."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="data/processed/metadata/domain_classification_sentences.csv",
        help="CSV path relative to project root. Use A4/A5 CSVs for those variants.",
    )
    parser.add_argument("--tag", type=str, required=True, help="Checkpoint name: <tag>.pt")
    ...
    model = ConvNeXtDomainClassifier(args.backbone, num_classes=2, pretrained=True).to(device)
    print("Model backbone:", args.backbone)

    if args.freeze_backbone:
        for name, param in model.backbone.named_parameters():
            param.requires_grad = name.startswith("classifier.2")
