...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train ViT-based domain classifier (IAM vs Emuru) on baseline / A1–A5 CSVs."
    )
    parser.add_argument(
        "--project-root",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Tag for checkpoint naming, e.g. 'vit_baseline', 'vit_A1_geo', etc.",
    )
    parser.add_argument(
        "--backbone",
        type=str,
        default="vit_b_16",
        help="ViT backbone: vit_b_16, vit_l_16",
    )
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="If set, freeze ViT backbone and train only the final head.",
    )
    parser.add_argument(
        "--augment-train",
        action="store_true",
    ...
    if args.freeze_backbone:
        # Freeze everything except heads.head
        for name, param in model.backbone.named_parameters():
            if name.startswith("heads.head"):
                param.requires_grad = True
            else:
                param.requires_grad = False
        print("Backbone frozen; only final head will be trained.")
