...
class ClipDomainClassifier(nn.Module):
    ...
    def extract_features(self, pixel_values: torch.Tensor) -> torch.Tensor:
        """CLIP pooled features (before linear head)."""
        out = self.clip_vision(pixel_values=pixel_values)
...
def compute_tsne_features(
    ...
    for batch in loader:
        ...
        feats = model.extract_features(x)  # (B, hidden)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze CLIP-head checkpoint (test metrics + optional t-SNE). Saves outputs to newAnalysis/."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Tag used for checkpoint name: newCheckpoints/clip_domain_<tag>.pt",
    )
    ...
    ckpt_path = (project_root / args.ckpt_dir / f"clip_domain_{args.tag}.pt").resolve()
    ...
    out_csv = out_dir / f"clip_domain_{args.tag}_test_results.csv"
    ...
    if args.tsne:
        ...
        out_tsne_csv = out_dir / f"clip_domain_{args.tag}_tsne.csv"
        tsne_df.to_csv(out_tsne_csv, index=False)
        print("Saved t-SNE CSV to:", out_tsne_csv)

        out_tsne_npy = out_dir / f"clip_domain_{args.tag}_tsne_embedding.npy"
