...
def create_dataloaders(
    ...
    if "train" in splits:
        train_ds = DomainClassificationDataset(
            ...
            photo_aug_train=photo_aug_train,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train DomainCNN (IAM vs Emuru) for Baseline/A1–A5.")
    parser.add_argument("--project-root", type=str, default=str(Path(__file__).resolve().parent))
    parser.add_argument("--csv-path", type=str, required=True)
    parser.add_argument("--tag", type=str, required=True, help="Checkpoint name: domain_cnn_<tag>.pt")
    ...
    ckpt_path = ckpt_dir / f"domain_cnn_{args.tag}.pt"
    ...
    train_loader, val_loader, test_loader = create_dataloaders(
        ...
        photo_aug_train=args.photo_augment_train,
