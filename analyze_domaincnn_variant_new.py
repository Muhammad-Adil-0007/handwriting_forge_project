...
def create_test_loader(
    ...
    """
    DomainCNN uses the grayscale/padded dataset (DomainClassificationDataset).
    normalize_all is kept for interface symmetry; it will only matter if your dataset supports it.
    """
...
def evaluate(
    ...
    """
    Run inference on the test loader and return DataFrame with:
...
def compute_tsne_features(
    ...
    """
    Extract DomainCNN features from conv_block + global average pooling,
    then compute t-SNE.
    """
    ...
    for batch in loader:
        ...
        # Feature extraction (matches forward structure):
        feat_map = model.conv_block(images)          # (B, 128, H', W')
        feats = feat_map.mean(dim=(2, 3))            # GAP -> (B, 128)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze a DomainCNN checkpoint (test metrics + optional t-SNE). Saves outputs to newAnalysis/."
    )
    parser.add_argument(
        "--project-root",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Run tag, used for checkpoint filename and output naming (e.g., domain_cnn_A4_realistic).",
    )
    parser.add_argument(
        "--ckpt-dir",
        type=str,
        default="newCheckpoints",
        help="Directory containing checkpoints (default: newCheckpoints).",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default="newAnalysis",
        help="Directory to save analysis outputs (default: newAnalysis).",
    )
    parser.add_argument("--batch-size", type=int, default=8, help="Keep small to avoid OOM with padded widths.")
    parser.add_argument("--num-workers", type=int, default=4)
    parser.add_argument("--device", type=str, default="auto", help="auto|cpu|cuda")
    parser.add_argument(
        "--normalize-all",
        action="store_true",
        help="If your dataset supports it, apply contrast normalization on all splits (A3-style).",
    )
