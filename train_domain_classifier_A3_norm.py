...
BEST_MODEL_PATH = CHECKPOINT_DIR / "domain_cnn_best_A3_norm.pt"
...
def create_dataloaders(
    ...
    For A3_domain_norm:
    - normalize_all=True on ALL splits (train/val/test)
    - no geometric or photometric augmentation.
    """
    ...
    if "train" in splits:
        train_ds = DomainClassificationDataset(
            ...
            normalize_all=True,    # <-- A3: deterministic normalization
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train IAM vs Emuru domain classifier with A3_domain_norm preprocessing"
    )
