...
class ConvNeXtDomainClassifier(nn.Module):
    """ConvNeXt backbone with 2-class head (matches training)."""
...
def compute_tsne_features(
    ...
    for batch in loader:
        ...
        # ConvNeXt features (pre-classifier)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze ConvNeXt checkpoint (test + optional t-SNE) -> newAnalysis/")
    parser.add_argument("--project-root", type=str, default=str(Path(__file__).resolve().parent))
    parser.add_argument("--csv-path", type=str, required=True)
    parser.add_argument("--tag", type=str, required=True, help="Checkpoint name: newCheckpoints/<tag>.pt")
