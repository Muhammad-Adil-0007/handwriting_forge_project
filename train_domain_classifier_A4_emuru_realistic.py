#!/usr/bin/env python
"""
Train IAM vs Emuru domain classifier on A4_emuru_realistic data.

- Uses domain_classification_sentences_A4_emuru_realistic.csv
- No geometric/photometric/normalization augmentation (pure baseline on new data).
"""
...
BEST_MODEL_PATH = CHECKPOINT_DIR / "domain_cnn_best_A4_emuru_realistic.pt"
...
def create_dataloaders(
    ...
    """
    Create train / val / test DataLoaders from the A4 CSV.
    No augmentations, no normalization — the A4 images are already 'baked'.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train IAM vs Emuru domain classifier with A4_emuru_realistic images"
    )
    ...
    parser.add_argument(
        "--csv-path",
        type=str,
        default="data/processed/metadata/domain_classification_sentences_A4_emuru_realistic.csv",
        help="Path to A4 domain classification CSV (relative to project root)",
    )
