...
BEST_MODEL_PATH = CHECKPOINT_DIR / "domain_cnn_best_A2_photo.pt"
...
def create_dataloaders(
    ...
    For A2_photo_both:
    - train_ds uses photo_aug_train=True (photometric augmentation).
    ...
    if "train" in splits:
        train_ds = DomainClassificationDataset(
            ...
            split="train",
            augment_train=False,      # no geometric aug here
            photo_aug_train=True,     # photometric aug ON
        )
    ...
    if val_split_name is not None:
        val_ds = DomainClassificationDataset(
            ...
            augment_train=False,
            photo_aug_train=False,    # no aug at eval
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train IAM vs Emuru domain classifier with A2_photo_both augmentation"
    )
